  ...
  return (
    <div className="product-card rounded-lg bg-grey-light text-base shadow-lg relative">
      <div className="card-content relative bg-cover bg-center rounded-t-lg h-36">
        { product.image ? <Image src={ product.image } alt={ product.name } fill={ true } objectFit={ 'cover' }
                                 className={ 'rounded-t-lg' } /> : null }
        <div style={ { background: 'linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4))' } }
             className="relative rounded-t-lg z-10 p-2 h-36 flex flex-col justify-between">
          ...
          <div className="content-footer text-lg text-white">
            ...
            <p><span className="font-semibold">Propriétaire :</span> { product.isOwner ? 'Moi' : product.owner.surname }
            </p>
          </div>
        </div>
      </div>
      <div className="card-footer p-2">
        ...
        <div className="flex justify-center mt-4">
          { product.showButton ?
            <Button text={ product.buttonText } onClick={ () => {
              action(product.popupText);
            } } />
            : null }
        </div>
      </div>
